# Access Watch React UI components

This repository contains reusable components to be shared between the different react apps at Access Watch.
Components are shared in a library which allow easy per component import in the other apps.
You may see a view of all the available components by running npm run start.
You should add any new component to this view.

## Notes
Unfortunately, a CSS per component structure is a bit tougher to achieve. For now it is needed to include the full bundle.css, even though your project might only use one component.
A beginning of a solution was developed to compile a per component based CSS, but dependencies resolving is harder than it seems with components depending on other components inside the bundle.
