...
const lazyLoad = _ => {
  ...
  funcs.openChat = _ => {
    ...
    if (!openChatEventWaiting) {
      ...
      eventEmitter.once(KWIKCHAT_READY, _ => {
  ...
  funcs.kcBodyNodeObserver = cb => new MutationObserver(mutations => {
  ...
  funcs.whenKwikchatReady = _ => new Promise(resolve => {
    if (kwikChatReady) {
      resolve(true);
    }
  ...
  funcs.loadChat = _ => {
    if (loadingChat || kwikChatReady) {
      ...
      s.setAttribute('src', 'https://cdn.chatkwik.com/cdn/widget/f945ebd210e1a0d8eb6a669f5591b60e7015631fe4c5a78d236f80461e8015ea');
      s.onload = () => {
        funcs.whenKwikchatReady().then(() => {
          Array.prototype.forEach.call(
            document.getElementById('kwikchat').getElementsByClassName('header__button'),
            button => { button.addEventListener('click', e => { e.preventDefault(); chatOpened = false; }); }
          );
        });
      };
