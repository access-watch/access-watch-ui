...
const percentageToPath = ({ cx, cy, r }) => (percentage, percentageOffset) => {
  ...
  return `M ${end.x} ${end.y} A ${r} ${r}, 0, ${largeArc}, 0, ${start.x} ${
    start.y
  } L ${cx} ${cy} Z`;
};
  ...
  render() {
    ...
    return (
      <svg
        ...
        viewBox="0 0 200 200"
      >
        <circle cx="100" cy="100" r="100px" className="pie-chart__empty" />
        {distributions.map(({ name, percentage }, i) => (
          <path
            className={`pie-chart__chunk pie-chart__chunk--${name}`}
            key={name}
            d={percentageToPath({ cx: 100, cy: 100, r: 100 })(
              percentage,
              (0.75 +
                distributions
                  .slice(0, i)
                  .reduce((val, d) => val + d.percentage, 0)) %
                1
            )}
            onClick={() => {
        ...
        <circle cx="100" cy="100" r="45px" className="pie-chart__mask" />
      </svg>
    );
  }
}
